...
public final class InventoryTelaahResep extends javax.swing.JDialog {
    ...
    public InventoryTelaahResep(java.awt.Frame parent, boolean modal) {
        ...
        tabMode=new DefaultTableModel(null,new Object[]{
            "No.Resep","Tgl Telaah","Jam Telaah","No.Rawat","No.R.M.","Nama Pasien","Umur","J.K.","Tgl.Lahir","Kode Dokter","Nama Dokter",
            "Status","Resep Lengkap","Identitas Pasien Sesuai","Obat Tepat","Campuran Obat Stabil",
            "Jumlah Tepat","Dosis/Kekuatan/Frekuensi","Rute Pemberian Tepat","Tidak Ada Interaksi Obat",
            "Tidak Ada Duplikasi","Tidak Ada Alergi/Kontradikasi","Benar Pasien","Benar Obat","Benar Dosis Pemberian",
            "Benar Rute pemberian","Benar Waktu Pemberian","Nama Obat dan Ketentuan Dosis","Aturan Pakai","Indikasi/Kegunaan Obat",
            "Penyimpaan Obat","Cara Pemakaian Obat","Waktu Penggunaan Obat","Kemungkinan Alergi Obat","NIP","Petugas Farmasi"
        }){
    ...
    public void tampil() {
        ...
        try{
            ps=koneksi.prepareStatement(
                "select telaah_resep_obat.no_resep,telaah_resep_obat.tgl,telaah_resep_obat.jam_telaah,telaah_resep_obat.no_rawat,reg_periksa.no_rkm_medis,"
                        + "pasien.nm_pasien,pasien.umur, pasien.jk,pasien.tgl_lahir,reg_periksa.kd_dokter,dokter.nm_dokter,telaah_resep_obat.`status`,telaah_resep_obat.resep,"
                        + "telaah_resep_obat.identitas,telaah_resep_obat.obat,telaah_resep_obat.campuran,telaah_resep_obat.jumlah,telaah_resep_obat.dosis,"
                        + "telaah_resep_obat.rute,telaah_resep_obat.tidak1,telaah_resep_obat.tidak2,telaah_resep_obat.tidak3,telaah_resep_obat.benar1,"
                        + "telaah_resep_obat.benar2,telaah_resep_obat.benar3,telaah_resep_obat.benar4,telaah_resep_obat.benar5,telaah_resep_obat.NOKDO,telaah_resep_obat.AP,"
                        + "telaah_resep_obat.IKO,telaah_resep_obat.PO,telaah_resep_obat.CPO,telaah_resep_obat.WPO,telaah_resep_obat.KAO,telaah_resep_obat.nip,"
                        + "petugas.nama from telaah_resep_obat INNER JOIN resep_obat on resep_obat.no_resep=telaah_resep_obat.no_resep "
                        + "INNER JOIN reg_periksa on reg_periksa.no_rawat=telaah_resep_obat.no_rawat "
                        + "INNER JOIN pasien on pasien.no_rkm_medis=reg_periksa.no_rkm_medis "
                        + "INNER JOIN dokter on dokter.kd_dokter=reg_periksa.kd_dokter "
                        + "INNER JOIN petugas on petugas.nip=telaah_resep_obat.nip "
                        + "where resep_obat.tgl_perawatan between ? and ? "+
                (TCari.getText().equals("")?"":"and (telaah_resep_obat.no_resep like ? or resep_obat.no_rawat like ? or reg_periksa.no_rkm_medis like ? or "+
                "pasien.nm_pasien like ? or resep_obat.kd_dokter like ? or dokter.nm_dokter like ? or resep_obat.status like ? or telaah_resep_obat.nip like ? or "+
            ...
            try {
                ...
                while(rs.next()){
                    tabMode.addRow(new String[]{
                        rs.getString("no_resep"),rs.getString("tgl"),rs.getString("jam_telaah"),rs.getString("no_rawat"),rs.getString("no_rkm_medis"),
                        rs.getString("nm_pasien"),rs.getString("umur"),rs.getString("jk"),rs.getString("tgl_lahir"),rs.getString("kd_dokter"),
                        rs.getString("nm_dokter"),rs.getString("status").replace("r","R"),rs.getString("resep"),rs.getString("identitas"),
                        rs.getString("obat"),rs.getString("campuran"),rs.getString("jumlah"),rs.getString("dosis"),
                        rs.getString("rute"),rs.getString("tidak1"),rs.getString("tidak2"),
                        rs.getString("tidak3"),rs.getString("benar1"),rs.getString("benar2"),
                        rs.getString("benar3"),rs.getString("benar4"),rs.getString("benar5"),
                        rs.getString("NOKDO"),rs.getString("AP"),rs.getString("IKO"),rs.getString("PO"),
                        rs.getString("CPO"),rs.getString("WPO"),rs.getString("KAO"),
