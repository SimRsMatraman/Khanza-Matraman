...
public class DlgPermintaan extends javax.swing.JDialog {
    ...
    private String[] jumlah, kodebarang, minp, maxp, stokb, stokasal, namabarang, satuan, jenis, kategori, golongan, keterangan, ket;
    private double[] j;
    ...
    private final java.util.Set<Integer> editedRows = new java.util.HashSet<>();
    ...
    public DlgPermintaan(java.awt.Frame parent, boolean modal) {
        ...
        Object[] judul = {"Kode Barang", "Nama Barang", "Satuan", "Jenis Obat", "Kategori", "Golongan", "Min. Permintaan", "Max. Permintaan", "Keterangan", "Stok Unit Peminta", "Jml", "Stok Unit Asal", "Keterangan", "Catatan", "Fix"};
...
//        tbDokter.setDefaultRenderer(Object.class, warna);
        try {
            if(koneksiDB.AKTIFKANWARNARALAN().equals("yes")){
                tbDokter.setDefaultRenderer(Object.class, new WarnaTableApt(nmgudangasal, kdgudangTujuan, nmgudangTujuan));
            }else{
                tbDokter.setDefaultRenderer(Object.class, warna);
            }
        } catch (Exception e) {
        ...
        // Tambahkan listener agar tombol aktif otomatis saat kdgudangbangsal diisi
        kdgudangasal.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
            private void updateButtonState() {
                boolean adaIsi = !kdgudangasal.getText().trim().isEmpty();
                btnSuplier.setEnabled(adaIsi);
            }
...
*/
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        for (i = 0; i < tbDokter.getRowCount(); i++) {
            if (!tbDokter.getValueAt(i, 10).toString().equals("")) {
                jml++;
            }
        }
...
//    }
    ...
    private void tampil() {
        ...
        for(i=0;i<tbDokter.getRowCount();i++){
            if (!tbDokter.getValueAt(i,10).toString().equals("")) {
                try {
                    j[z] = Double.parseDouble(tbDokter.getValueAt(i,10).toString());
                } catch (Exception e) {
                    j[z] = 0;
                } 
                kodebarang[z] = tbDokter.getValueAt(i,0).toString();
                namabarang[z] = tbDokter.getValueAt(i,1).toString();
                satuan[z] = tbDokter.getValueAt(i,2).toString();
                jenis[z] = tbDokter.getValueAt(i,3).toString();
                kategori[z] = tbDokter.getValueAt(i,4).toString();
                golongan[z] = tbDokter.getValueAt(i,5).toString();
                // 👇 Ubah jadi String, bukan double
                minp[z] = tbDokter.getValueAt(i,6).toString();
                maxp[z] = tbDokter.getValueAt(i,7).toString();
                try {
                    keterangan[z] = tbDokter.getValueAt(i,8).toString();
                } catch (Exception e) {
                    keterangan[z] = "";
                }
                stokb[z] = tbDokter.getValueAt(i,9).toString();
                stokasal[z] = tbDokter.getValueAt(i,11).toString();
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    try {
                        ...
                        tabMode.addRow(new Object[]{
                            rs.getString(1), rs.getString(2), rs.getString(3),
                            rs.getString(4), rs.getString(5), rs.getString(6), rs.getString(8), rs.getString(9), "", Valid.SetAngkaPlain(stok), "", rs.getString(7), ""
                        });
                    iyem = iyem + "{\"KodeBarang\":\"" + rs.getString(1) + "\",\"NamaBarang\":\"" + rs.getString(2).replaceAll("\"", "") + "\",\"Satuan\":\"" + rs.getString(3) + "\",\"JenisObat\":\"" + rs.getString(4) + "\",\"Kategori\":\"" + rs.getString(5) + "\",\"Golongan\":\"" + rs.getString(6) + "\",\"Min. Permintaan\":\"" + rs.getString(8) + "\",\"Max.Permintaan\":\"" + rs.getString(9) + "\",\"Stok Bangsal\":\"" + Valid.SetAngkaPlain(stok) + "\",\"Stok Asal\":\"" + Valid.SetAngkaPlain(jumlahbeli1) + "\"},";
                }
            } catch (Exception e) {
...
//            }
    ...
    private void getCekStok() {
        if(tbDokter.getSelectedRow()!= -1){
            if(STOKKOSONGRESEP.equals("no")){
                try {
                    if(Double.parseDouble(tbDokter.getValueAt(tbDokter.getSelectedRow(),10).toString())>0){
                        if(Valid.SetAngka(tbDokter.getValueAt(tbDokter.getSelectedRow(),10).toString())>Valid.SetAngka(tbDokter.getValueAt(tbDokter.getSelectedRow(),11).toString())){
                            JOptionPane.showMessageDialog(rootPane,"Maaf permintaan melebihi stok bangsal asal..!!");
                            tbDokter.setValueAt("",tbDokter.getRowCount(),10);
                        }
                    }
                } catch (Exception e) {
    ...
    private void getCekStokMin() {
        if(tbDokter.getSelectedRow()!= -1){
            if(STOKKOSONGRESEP.equals("no")){
                try {
                    if(Double.parseDouble(tbDokter.getValueAt(tbDokter.getSelectedRow(),10).toString())>0){
                        if(Valid.SetAngka(tbDokter.getValueAt(tbDokter.getSelectedRow(),10).toString())<Valid.SetAngka(tbDokter.getValueAt(tbDokter.getSelectedRow(),6).toString())){
    ...
    private void getCekStokMax() {
        if(tbDokter.getSelectedRow()!= -1){
            if(STOKKOSONGRESEP.equals("no")){
                try {
                    if(Double.parseDouble(tbDokter.getValueAt(tbDokter.getSelectedRow(),10).toString())>0){
                        if(Valid.SetAngka(tbDokter.getValueAt(tbDokter.getSelectedRow(),10).toString())>Valid.SetAngka(tbDokter.getValueAt(tbDokter.getSelectedRow(),7).toString())){
    ...
    private void getCekStokAuto() {
        int rowCount = tbDokter.getRowCount();

        for (int row = 0; row < rowCount; row++) {
            try {
                Object val6  = tbDokter.getValueAt(row, 6);
                Object val7  = tbDokter.getValueAt(row, 7);
                Object val9  = tbDokter.getValueAt(row, 9);
                Object val11 = tbDokter.getValueAt(row, 11);

                if (val6 == null || val7 == null || val9 == null || val11 == null ||
                    val6.toString().trim().isEmpty() ||
                    val7.toString().trim().isEmpty() ||
                    val9.toString().trim().isEmpty() ||
                    val11.toString().trim().isEmpty()) {
                    tbDokter.setValueAt("", row, 10);
                    tbDokter.setValueAt("", row, 14);
                    continue;
                }

                double minPermintaan = Valid.SetAngka(val6.toString());
                double maxPermintaan = Valid.SetAngka(val7.toString());
                double stokPeminta   = Valid.SetAngka(val9.toString());
                double stokAsal      = Valid.SetAngka(val11.toString());
                ...
                if (stokPeminta < minPermintaan) {
                    double hasil = maxPermintaan - stokPeminta;
                    if (hasil < 0) hasil = 0;

                    if (hasil > stokAsal) hasil = stokAsal;

                    Object val10 = tbDokter.getValueAt(row, 10);

                    if (val10 == null || val10.toString().trim().isEmpty()) {
                        tbDokter.setValueAt(Valid.SetAngkaPlain(hasil), row, 10);
                    }

                    tbDokter.setValueAt(Valid.SetAngkaPlain(hasil), row, 14);

                    try {
                        double nilai10 = Valid.SetAngka(tbDokter.getValueAt(row, 10).toString());
                        if (nilai10 > maxPermintaan || nilai10 > stokAsal) {
                            tbDokter.setValueAt("", row, 10);
                        }
                    } catch (Exception ex) {
                        tbDokter.setValueAt("", row, 10);
        ...
        tbDokter.getModel().addTableModelListener(evt -> {
            if (evt.getColumn() == 10) {
                int editedRow = evt.getFirstRow();
                if (editedRow >= 0) {
