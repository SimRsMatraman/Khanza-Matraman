...
public final class DlgResepPulang extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        ppTelaahResepObat = new javax.swing.JMenuItem();
        ...
        ppTelaahResepObat.setBackground(new java.awt.Color(255, 255, 254));
        ppTelaahResepObat.setFont(new java.awt.Font("Tahoma", 0, 11)); // NOI18N
        ppTelaahResepObat.setForeground(new java.awt.Color(50, 50, 50));
        ppTelaahResepObat.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png"))); // NOI18N
        ppTelaahResepObat.setText("Telaah Resep Obat Pulang");
        ppTelaahResepObat.setToolTipText("");
        ppTelaahResepObat.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        ppTelaahResepObat.setHorizontalTextPosition(javax.swing.SwingConstants.RIGHT);
        ppTelaahResepObat.setName("ppTelaahResepObat"); // NOI18N
        ppTelaahResepObat.setPreferredSize(new java.awt.Dimension(225, 25));
        ppTelaahResepObat.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                ppTelaahResepObatActionPerformed(evt);
            }
        });
        jPopupMenu1.add(ppTelaahResepObat);
...
private void ChkInputActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ChkInputActionPerformed
  isForm();                
    ...
    private void ppTelaahResepObatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppTelaahResepObatActionPerformed
        //        if(TNoRw.getText().trim().equals("")){
            //            JOptionPane.showMessageDialog(null,"Maaf, Silahkan anda pilih dulu dengan menklik data pada table...!!!");
            //            TCari.requestFocus();
            //        }else{
            //            this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
            //            DlgTelaahObatPulang telaahpulang=new DlgTelaahObatPulang(null,false);
            //            telaahpulang.isCek();
            //            telaahpulang.setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
            //            telaahpulang.setLocationRelativeTo(internalFrame1);
            //            //            telaah.setNoRm(TNoRw.getText(),DTPCari2.getDate());
            //            telaahpulang.setNoRm(TNoRw.getText(),DTPCari2.getDate());
            //            telaahpulang.tampil();
            //            telaahpulang.emptTeks();
            //            telaahpulang.setVisible(true);
            //            this.setCursor(Cursor.getDefaultCursor());
            //        }
        if(TNoRw.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null,"Maaf, Silahkan anda pilih dulu dengan menklik data pada table...!!!");
    ...
    }//GEN-LAST:event_ppTelaahResepObatActionPerformed
    ...
    private void ppLembarObatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppLembarObatActionPerformed
        if(tabMode.getRowCount()==0){
            ...
            param.put("propinsirs",akses.getpropinsirs());
            param.put("tanggal",Sequel.cariIsi("select tanggal from telaah_resep_pulang where no_rawat=?",TNoRw.getText()));
            ...
            param.put("peresep",Sequel.cariIsi("select nm_dokter from dpjp_ranap INNER JOIN dokter ON dpjp_ranap.kd_dokter = dokter.kd_dokter where no_rawat=?",TNoRw.getText()));
            param.put("noresep",Sequel.cariIsi("select no_resep from telaah_resep_pulang where no_rawat=?",TNoRw.getText()));
            param.put("jam",Sequel.cariIsi("select jam from telaah_resep_pulang where no_rawat=?",TNoRw.getText()));
            param.put("logo",Sequel.cariGambar("select logo from setting"));
            //            param.put("nm_poli",Sequel.cariIsi("select nm_poli from poliklinik where kd_poli=?",Sequel.cariIsi("select kd_poli from reg_periksa where no_rawat=?",TNoRw.getText())));
            ...
            param.put("bb",Sequel.cariIsi("select berat from pemeriksaan_ranap where no_rawat=? order by tgl_perawatan desc",TNoRw.getText()));
            param.put("alergi",Sequel.cariIsi("select alergi from pemeriksaan_ranap where no_rawat=? order by tgl_perawatan desc",TNoRw.getText()));
            param.put("nm_poli",Sequel.cariIsi("select nm_bangsal from bangsal inner join kamar inner join kamar_inap on bangsal.kd_bangsal=kamar.kd_bangsal and kamar_inap.kd_kamar=kamar.kd_kamar where no_rawat=? order by tgl_masuk desc limit 1 ",TNoRw.getText()));
            param.put("resep",Sequel.cariIsi("select resep from telaah_resep_pulang where no_rawat=?",TNoRw.getText()));
            param.put("identitas",Sequel.cariIsi("select identitas from telaah_resep_pulang where no_rawat=?",TNoRw.getText()));
            param.put("obat",Sequel.cariIsi("select obat from telaah_resep_pulang where no_rawat=?",TNoRw.getText()));
            param.put("campuran",Sequel.cariIsi("select campuran from telaah_resep_pulang where no_rawat=?",TNoRw.getText()));
            param.put("jumlah",Sequel.cariIsi("select jumlah from telaah_resep_pulang where no_rawat=?",TNoRw.getText()));
            param.put("dosis",Sequel.cariIsi("select dosis from telaah_resep_pulang where no_rawat=?",TNoRw.getText()));
            param.put("rute",Sequel.cariIsi("select rute from telaah_resep_pulang where no_rawat=?",TNoRw.getText()));
            param.put("tidak1",Sequel.cariIsi("select tidak1 from telaah_resep_pulang where no_rawat=?",TNoRw.getText()));
            param.put("tidak2",Sequel.cariIsi("select tidak2 from telaah_resep_pulang where no_rawat=?",TNoRw.getText()));
            param.put("tidak3",Sequel.cariIsi("select tidak3 from telaah_resep_pulang where no_rawat=?",TNoRw.getText()));
            param.put("benar1",Sequel.cariIsi("select benar1 from telaah_resep_pulang where no_rawat=?",TNoRw.getText()));
            param.put("benar2",Sequel.cariIsi("select benar2 from telaah_resep_pulang where no_rawat=?",TNoRw.getText()));
            param.put("benar3",Sequel.cariIsi("select benar3 from telaah_resep_pulang where no_rawat=?",TNoRw.getText()));
            param.put("benar4",Sequel.cariIsi("select benar4 from telaah_resep_pulang where no_rawat=?",TNoRw.getText()));
            param.put("benar5",Sequel.cariIsi("select benar5 from telaah_resep_pulang where no_rawat=?",TNoRw.getText()));

            Valid.MyReport("rptLembarObatPulang.jasper",param,"::[ Lembar Pemberian Obat Pulang ]::");
            this.setCursor(Cursor.getDefaultCursor());
        }
    ...
    */
    ...
    private javax.swing.JMenuItem ppTelaahResepObat;
