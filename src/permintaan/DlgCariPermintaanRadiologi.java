...
public class DlgCariPermintaanRadiologi extends javax.swing.JDialog {
    ...
    public DlgCariPermintaanRadiologi(java.awt.Frame parent, boolean modal) {
        ...
        tbRadiologiRalan.setDefaultRenderer(Object.class, new WarnaTableRAD());
        ...
        tabMode3=new DefaultTableModel(null,new Object[]{
            "No.Permintaan","No.Rawat","Pasien","Permintaan","Jam","Sampel","Jam","Hasil","Jam","Kode Dokter","Dokter Perujuk","Kamar Terakhir","Informasi Tambahan","Diagnosis Klinis","Kode Bayar","Jenis Bayar","Tambahan","Status Bayar"
            }){
              @Override public boolean isCellEditable(int rowIndex, int colIndex){return false;}
        };
    ...
    private void initComponents() {
        ...
        BtnCloseIn4.setBounds(500, 30, 100, 30);
        ...
        TanggalPulang.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "26-01-2024 18:11:32" }));
        ...
        internalFrame6.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Update Tanggal Permintaan Radiologi ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 0, 11), new java.awt.Color(50, 50, 50))); // NOI18N
        ...
        if(TabPilihRawat.getSelectedIndex()==0){
            if(tbRadiologiRalan.getSelectedRow()!= -1){
                if(tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(),0).toString().trim().equals("")){
                    ...
                    if(Sequel.mengedittf("permintaan_radiologi","noorder=?","tgl_sampel=?,jam_sampel=?,tambahan=?",4,new String[]{
                        Valid.SetTgl(TanggalPulang.getSelectedItem()+""),TanggalPulang.getSelectedItem().toString().substring(11,19),Cmbstatus.getSelectedItem()+"",tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(),0).toString()
                    })==true){
                        ...
                        tbRadiologiRalan.setValueAt(Cmbstatus.getSelectedItem()+"",tbRadiologiRalan.getSelectedRow(),13);
                        WindowAmbilSampel.dispose();
                    }
                }
            }else{            
            ...
            if(tbRadiologiRanap.getSelectedRow()!= -1){
                if(tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(),0).toString().trim().equals("")){
                    ...
                    if(Sequel.mengedittf("permintaan_radiologi","noorder=?","tgl_sampel=?,jam_sampel=?,tambahan=?",4,new String[]{
                        Valid.SetTgl(TanggalPulang.getSelectedItem()+""),TanggalPulang.getSelectedItem().toString().substring(11,19),Cmbstatus.getSelectedItem()+"",tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(),0).toString()
                    })==true){
                        ...
                        tbRadiologiRanap.setValueAt(Cmbstatus.getSelectedItem()+"",tbRadiologiRanap.getSelectedRow(),13);
                        WindowAmbilSampel.dispose();
                    }
                }
            }else{            
        ...
        if(TabPilihRawat.getSelectedIndex()==0){
            if(tbRadiologiRalan.getSelectedRow()!= -1){
                if(tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(),0).toString().trim().equals("")){
                    ...
                    if(Sequel.mengedittf("permintaan_radiologi","noorder=?","noorder=?,tgl_permintaan=?",3,new String[]{
...
//              Jika Mau merubah jam juga
                        ...
                        WindowGanti.dispose();
                        TeksKosong();
                        tbRadiologiRalan.setValueAt(TNoPermintaanPR.getText(),tbRadiologiRalan.getSelectedRow(),0);
                        tbRadiologiRalan.setValueAt(Valid.SetTgl(TanggalPulang1.getSelectedItem()+""),tbRadiologiRalan.getSelectedRow(),3);
//              Jika Mau merubah jam juga
...
//              Jika Mau merubah jam juga
                        ...
                        TeksKosong();
                        tbRadiologiRalan.setValueAt(TNoPermintaanPR.getText(),tbRadiologiRalan.getSelectedRow(),0);
                        tbRadiologiRanap.setValueAt(Valid.SetTgl(TanggalPulang1.getSelectedItem()+""),tbRadiologiRanap.getSelectedRow(),3);
//              Jika Mau merubah jam juga
    ...
    private void tampil() {
        ...
        try {
            ...
            ps=koneksi.prepareStatement(
                    ...
                    "permintaan_radiologi.dokter_perujuk,dokter.nm_dokter,poliklinik.nm_poli,permintaan_radiologi.informasi_tambahan,permintaan_radiologi.diagnosa_klinis,permintaan_radiologi.tambahan,reg_periksa.status_bayar "+
...
//                    ps.setString(16,"%"+TCari.getText()+"%");
        ...
        try {
            ...
            if(cmbStatus.getSelectedIndex()==0){
                ps=koneksi.prepareStatement(
                        ...
                        "if(permintaan_radiologi.tgl_sampel='0000-00-00','',permintaan_radiologi.tgl_sampel) as tgl_sampel,if(permintaan_radiologi.jam_sampel='00:00:00','',permintaan_radiologi.jam_sampel) as jam_sampel,"+
                        "permintaan_radiologi.tgl_hasil,if(permintaan_radiologi.jam_hasil='00:00:00','',permintaan_radiologi.jam_hasil) as jam_hasil,"+
                        "permintaan_radiologi.dokter_perujuk,dokter.nm_dokter,ifnull(bangsal.nm_bangsal,'Ranap Gabung') as nm_bangsal,permintaan_radiologi.informasi_tambahan,permintaan_radiologi.diagnosa_klinis,permintaan_radiologi.tambahan,reg_periksa.status_bayar "+
                        "from permintaan_radiologi inner join reg_periksa on permintaan_radiologi.no_rawat=reg_periksa.no_rawat "+
                        "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                        "inner join dokter on permintaan_radiologi.dokter_perujuk=dokter.kd_dokter "+
                ...
                if(detik.equals("05")){
                    ...
                    if(formalarm.contains("ralan")){
                        ...
                        for(i=0;i<tbRadiologiRalan.getRowCount();i++){
                            if((!tbRadiologiRalan.getValueAt(i,0).toString().equals(""))&&tbRadiologiRalan.getValueAt(i,16).toString().equals("Cito")){
                                permintaanbaru++;
                            }
                        }
                    }
    ...
    private void autoNomor() {
        Valid.autoNomer3("select ifnull(MAX(CONVERT(RIGHT(permintaan_Radiologi.noorder,4),signed)),0) from permintaan_Radiologi where permintaan_Radiologi.tgl_permintaan='"+Valid.SetTgl(TanggalPulang1.getSelectedItem()+"")+"' ","PR"+Valid.SetTgl(TanggalPulang1.getSelectedItem()+"").replaceAll("-",""),4,TNoPermintaanPR);           
    }
