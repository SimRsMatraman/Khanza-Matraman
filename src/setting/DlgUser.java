...
public class DlgUser extends javax.swing.JDialog {
    ...
    public DlgUser(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"ID User","Nama User","Jabatan","Password","[J]ICD 10","[J]Obat Penyakit","[C]Dokter","[A]Jadwal Praktek","[C]Petugas","[M]Pasien","[A]Registrasi","[A]Tindakan Ralan",
                ...
                "[J]Pemeriksaan Fisik Ralan Per Penyakit","Hapus Data Salah","[L]Kirim Diet Satu Sehat","[L]Mapping Obat/Alkes/BHP Satu Sehat","[L]Kirim Medication Satu Sehat","[L]Kirim Medication Request Satu Sehat",
                "[L]Kirim Medication Dispense Satu Sehat","[L]Mapping Tindakan Radiologi Satu Sehat","[L]Kirim Service Request Radiologi Satu Sehat","[L]Kirim Specimen Radiologi Satu Sehat",
                "[L]Kirim Observation Radiologi Satu Sehat","[L]Kirim Diagnostic Report Radiologi Satu Sehat","[L]Mapping Tindakan Lab PK & MB Satu Sehat",
...
//                    "validasi_pengajuan_biaya='"+tbUser.getValueAt(i,964).toString()+"',"+
                    ...
                    "hapus_data_salah_kamar_inap='"+tbUser.getValueAt(i,966).toString()+"'")==true){
                    emptTeks();
                }
            }         
        }
...
//                                        "validasi_pengajuan_biaya='"+tbUser.getValueAt(barisdicopy,964).toString()+"',"+
                                        ...
                                        "hapus_data_salah_kamar_inap='"+tbUser.getValueAt(barisdicopy,966).toString()+"'");
                                }
                                userdicopy="";
                                copyhakakses="";
                                barisdicopy=-1;
    ...
    private void tampil() {        
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.checklist_post_operasi,user.dapur_pembelian,user.dapur_stok_keluar,user.satu_sehat_mapping_lab,user.dapur_riwayat_barang,user.permintaan_dapur,user.rekonsiliasi_obat,"+
                ...
                "user.rekap_biaya_registrasi,user.konfirmasi_rekonsiliasi_obat,user.satu_sehat_kirim_observation_radiologi,user.satu_sehat_kirim_diagnosticreport_radiologi,user.satu_sehat_kirim_clinicalimpression,user.penilaian_pasien_terminal,"+
                "user.surat_persetujuan_rawat_inap,user.monitoring_reaksi_tranfusi,user.satu_sehat_kirim_specimen_radiologi,user.penilaian_korban_kekerasan,user.penilaian_lanjutan_resiko_jatuh_lansia,"+
                "user.penilaian_pasien_penyakit_menular,user.mpp_skrining,user.satu_sehat_mapping_radiologi,user.satu_sehat_kirim_servicerequest_radiologi,user.edukasi_pasien_keluarga_rj,user.pemantauan_pews_dewasa,user.penilaian_tambahan_bunuh_diri,"+
                ...
                "user.sisa_diet_pasien,user.penilaian_awal_medis_ralan_bedah_mulut,user.satu_sehat_kirim_medicationdispense,user.penilaian_pasien_keracunan,user.pemantauan_meows_obstetri,user.catatan_adime_gizi,"+
                "user.pengajuan_biaya,user.penilaian_awal_keperawatan_ralan_geriatri,user.satu_sehat_kirim_medicationrequest,user.master_masalah_keperawatan_geriatri,user.master_rencana_keperawatan_geriatri,"+
                "user.checklist_kriteria_masuk_hcu,user.checklist_kriteria_keluar_hcu,user.satu_sehat_kirim_medication,user.penilaian_risiko_dekubitus,user.master_menolak_anjuran_medis,user.penolakan_anjuran_medis,"+
                ...
                "user.checklist_kriteria_keluar_icu,user.akses_dokter_lain_rawat_jalan,user.follow_up_dbd,user.penilaian_risiko_jatuh_neonatus,user.satu_sehat_kirim_diet,user.satu_sehat_mapping_obat,"+
...
//                "user.validasi_pengajuan_biaya,"+
                "user.pemeriksaan_fisik_ralan_per_penyakit, user.hapus_data_salah_kamar_inap from user order by AES_DECRYPT(user.id_user,'nur')");
            try {
                rs=ps.executeQuery();
                while(rs.next()){
                    user="";
...
//                               rs.getBoolean("validasi_pengajuan_biaya"),
                               ...
                               rs.getBoolean("hapus_data_salah_kamar_inap")      
                            });
                        }   
                    } catch (Exception e) {
                        tabMode.addRow(new Object[]{rs.getString(1),
