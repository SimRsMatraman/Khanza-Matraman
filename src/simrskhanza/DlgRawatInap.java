...
import rekammedis.RMDataICRanap;
...
//                            KdDok2.requestFocus();
    ...
    private void initComponents() {
        ...
        BtnEWS1.setBounds(660, 290, 100, 30);
        ...
        DTPTgl.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "12-03-2025" }));
                        ...
                        if(tbPemeriksaan.getValueAt(i,0).toString().equals("true")){
                            if(akses.getkode().equals("Admin Utama")){
                                ...
                                if(akses.getkode().equals(tbPemeriksaan.getValueAt(i,22).toString())){
                                    JOptionPane.showMessageDialog(null,"Hanya bisa dihapus admin");
//                                    Sequel.queryu("delete from pemeriksaan_ranap where no_rawat='"+tbPemeriksaan.getValueAt(i,1).toString()+
//                                            "' and tgl_perawatan='"+tbPemeriksaan.getValueAt(i,4).toString()+
//                                            "' and jam_rawat='"+tbPemeriksaan.getValueAt(i,5).toString()+"' ");
//                                    tabModePemeriksaan.removeRow(i);
...
//            resume.setNoRm(TNoRw.getText(),DTPCari2.getDate(),Sequel.cariIsi("select reg_periksa.kd_pj from reg_periksa where reg_periksa.no_rawat=?",TNoRw.getText()),TNoRM.getText());
    ...
    private void BtnCatatanObatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnCatatanObatActionPerformed
        if(TPasien.getText().trim().equals("")||TNoRw.getText().trim().equals("")){
            ...
            this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
            DlgCatatanPemberianObatRanap form = new DlgCatatanPemberianObatRanap(null, false);
            form.isCek();
            form.setSize(internalFrame1.getWidth(), internalFrame1.getHeight());
            form.setLocationRelativeTo(internalFrame1);
            form.setVisible(true);
    ...
    private void BtnAPSRanapActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnAPSRanapActionPerformed
        if(TNoRw.getText().trim().equals("")){
            ...
            this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
            SuratAPSRanap form=new SuratAPSRanap(null,false);
            form.isCek();
            form.setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
            form.setLocationRelativeTo(internalFrame1);
            form.setVisible(true);
...
//            NamaDokter.setText(tbPemeriksaan.getValueAt(tbPemeriksaan.getSelectedRow(),20).toString());
            ...
            if(tbPemeriksaan.getValueAt(tbPemeriksaan.getSelectedRow(),19).toString().equals(akses.getkode()))
            {
