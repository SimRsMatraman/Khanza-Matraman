 ...
 * @author dosen
 */
public final class DlgPeriksaFisio extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public DlgPeriksaFisio(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        listPetugas();
        
        Object[] row={"P","Kode Periksa","Nama Pemeriksaan","Material","BHP","Tarif Perawat","KSO","Manajemen","Total"};
...
//            }
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Input Data Pemeriksaan Fisioterapi ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Segoe UI", 0, 12), new java.awt.Color(50, 50, 50))); // NOI18N
        ...
        FormInput.setPreferredSize(new java.awt.Dimension(560, 220));
        ...
        PanelInput.setPreferredSize(new java.awt.Dimension(560, 220));
        ...
        PanelInput.add(listDok);
        listDok.setBounds(215, 165, 280, 22);
        ...
        PanelInput.add(label30);
        label30.setBounds(150, 165, 70, 23);
...
private void ChkJlnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ChkJlnActionPerformed
    ...
    private void InformasiTambahanKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_InformasiTambahanKeyPressed
        Valid.pindah(evt,InformasiTambahan,DiagnosisKlinis);
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgPeriksaFisio dialog = new DlgPeriksaFisio(new javax.swing.JFrame(), true);
    ...
    private void tampil() {         
        try{
            file=new File("./cache/periksafisio.iyem");
            file.createNewFile();
            fileWriter = new FileWriter(file);
            iyem=""; 
        
            pspemeriksaan=koneksi.prepareStatement(
                    "select jns_perawatan.kd_jenis_prw,jns_perawatan.nm_perawatan,material,bhp,tarif_tindakanpr,kso,menejemen,total_byrpr from jns_perawatan where jns_perawatan.status='1' and jns_perawatan.kd_jenis_prw LIKE 'FT%' order by jns_perawatan.kd_jenis_prw");
            try {
                rs=pspemeriksaan.executeQuery();
                while(rs.next()){
                    iyem=iyem+"{\"KodePeriksa\":\""+rs.getString(1)+"\",\"NamaPemeriksaan\":\""+rs.getString(2).replaceAll("\"","")+"\",\"material\":\""+rs.getString(3)+"\",\"bhp\":\""+rs.getString(4)+"\",\"tarifPerawat\":\""+rs.getString(5)+"\",\"kso\":\""+rs.getString(6)+"\",\"manajemen\":\""+rs.getString(7)+"\",\"total\":\""+rs.getString(8)+"\"},";
                }
            } catch (Exception e) {
            ...
            fileWriter.write("{\"periksafisio\":["+iyem.substring(0,iyem.length()-1)+"]}");
            fileWriter.flush();
            fileWriter.close();
            iyem=null;
        }catch(Exception e){
    ...
    private void tampil2() {         
        try{
            ...
            myObj = new FileReader("./cache/periksafisio.iyem");
            root = mapper.readTree(myObj);
            response = root.path("periksafisio");
                    ...
                    for(JsonNode list:response){
                        if(list.path("KodePeriksa").asText().toLowerCase().contains(TCariPeriksa.getText().toLowerCase())||list.path("NamaPemeriksaan").asText().toLowerCase().contains(TCariPeriksa.getText().toLowerCase())){
                            tabMode.addRow(new Object[]{
                                false,list.path("KodePeriksa").asText(),list.path("NamaPemeriksaan").asText(),list.path("material").asText(),list.path("bhp").asText(),list.path("tarifPerawat").asText(),list.path("kso").asText(),list.path("manajemen").asText(),list.path("total").asText()
                            });
                        }
                    }
                }
    ...
    public void setNoRm(String norwt,String posisi){
        ...
        isPsien();
        try {
            if(Valid.daysOld("./cache/periksafisio.iyem")<3){
                tampil2();
            }
        } catch (Exception e) {
        }
    ...
    private void isForm(){
        if(ChkInput.isSelected()==true){
            ...
            FormInput.setPreferredSize(new Dimension(WIDTH,220));
    ...
    public void tampil(String order) {         
        try{
            pspemeriksaan=koneksi.prepareStatement(
                    "select jns_perawatan.kd_jenis_prw,jns_perawatan.nm_perawatan,material,bhp,tarif_tindakanpr,kso,menejemen,total_byrpr "+
                    "from jns_perawatan inner join permintaan_pemeriksaan_fisio on jns_perawatan.kd_jenis_prw=permintaan_pemeriksaan_fisio.kd_jenis_prw "+
                    "where permintaan_pemeriksaan_fisio.noorder=? and jns_perawatan.kd_jenis_prw LIKE 'FT%' order by jns_perawatan.kd_jenis_prw");            
            try {
                pspemeriksaan.setString(1,order);
                rs=pspemeriksaan.executeQuery();                
                while(rs.next()){                
                    tabMode.addRow(new Object[]{true,rs.getString(1),rs.getString(2),rs.getDouble(3),rs.getDouble(4),rs.getDouble(5),rs.getDouble(6),rs.getDouble(7),rs.getDouble(8)});
                }
            } catch (Exception e) {
                System.out.println("Notifikasi 1 : "+e);
            } finally{
