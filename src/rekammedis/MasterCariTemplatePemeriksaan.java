...
public final class MasterCariTemplatePemeriksaan extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        Scroll12.setBounds(16, 2600, 700, 240);
                ...
                try {
                    ...
                    try {
                        ...
                        while(rs.next()){
                            ...
                            //});
                        ListObat=rs.getString("kd_penyakit");
                        ListObat1=rs.getString("nm_penyakit");
                        namaObat=namaObat+" "+" "+ListObat+" "+ListObat1+",  "+"\n";
                        }
                        Asesmen.append(namaObat.length()>0 ? namaObat.substring(0,namaObat.length()-1):"");
                    } catch (Exception e) {
                    ...
                    ps=koneksi.prepareStatement(
                            ...
                            "databarang.kapasitas,databarang.letak_barang as obat from template_pemeriksaan_dokter_resep inner join databarang on template_pemeriksaan_dokter_resep.kode_brng=databarang.kode_brng inner join kodesatuan on kodesatuan.kode_sat=databarang.kode_sat "+
                    ...
                    try {
                        ...
                        while(rs.next()){
                            ...
                            //});
                            ListObat=rs.getString("obat");
                            ListObat1=rs.getString("jml");
                            ListObat2=rs.getString("satuan");
                            ListObat3=rs.getString("aturan_pakai");
                            namaObat=namaObat+" "+" "+ListObat+" "+ListObat1+" "+ListObat2+" "+ListObat3+",  "+"\n";
                        }
                        Plan.append(namaObat.length()>0 ? namaObat.substring(0,namaObat.length()-1):"");
                    } catch (Exception e) {
                        System.out.println("Notif : "+e);
                    } finally{
                        if(rs!=null){
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(Asesmen.getText().trim().equals("")){
            ...
            if(tbDokter.getSelectedRow()>-1){
                ...
                sukses=true;
                if(Sequel.menyimpantf2("pemeriksaan_ralan","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","Data",24,new String[]{
                        noperawatan,tanggaldilakukan,jamdilakukan,"","","","","","","","Compos Mentis 15-14",Subjek.getText(),Objek.getText(),"","-",Plan.getText(),Asesmen.getText(),"","","","","",kodedokter,Instruksi.getText()}
                    )==true){
                    ...
                    if(tabModeRadiologi.getRowCount()>0){
                        ...
                        if(Sequel.menyimpantf2("permintaan_radiologi","?,?,?,?,?,?,?,?,?,?,?,?,?","No.Permintaan Radiologi",13,new String[]{
                                nomor,noperawatan,tanggaldilakukan,jamdilakukan,"0000-00-00","00:00:00","0000-00-00","00:00:00",kodedokter,"ralan","-",Asesmen.getText(),"No Alarm"
                            })==true){
