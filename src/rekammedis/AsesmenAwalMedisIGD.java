 ...
 * @author perpustakaan
 */
public final class AsesmenAwalMedisIGD extends javax.swing.JDialog {
...
//    private SimpleDateFormat tanggalNow = new SimpleDateFormat("dd-MM-yyyy");
     ...
     * @param parent
     * @param modal */
    public AsesmenAwalMedisIGD(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
...
//        isMenu();
    ...
    private void initComponents() {
        ...
        scrollPane12.setBounds(30, 1620, 730, 43);
        ...
        BtnEdit1.setBounds(170, 810, 160, 30);
        ...
        PanelWall.setBounds(30, 850, 870, 340);
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(TNoRM.getText().trim().equals("")){
            ...
            if(Sequel.menyimpantf("asesmen_medis_igd","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","No.Rawat",41,new String[]{
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        if(tbObat.getSelectedRow()>-1){
            if(Sequel.queryu2tf("delete from asesmen_medis_igd where no_rawat=?",1,new String[]{
    ...
    */
    ...
    public void tampil() {
        ...
        try{
            if(TCari.getText().equals("")){
                ps=koneksi.prepareStatement(
                        "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,if(pasien.jk='L','Laki-Laki','Perempuan') as jk,pasien.tgl_lahir,asesmen_medis_igd.tanggal,"+
                        "asesmen_medis_igd.jam,asesmen_medis_igd.kd_dokter,asesmen_medis_igd.anamnesis,asesmen_medis_igd.hubungan,asesmen_medis_igd.keluhan_utama,asesmen_medis_igd.rps,asesmen_medis_igd.rpk,asesmen_medis_igd.rpd,asesmen_medis_igd.rpo,asesmen_medis_igd.alergi,"+
                        "asesmen_medis_igd.keadaan,asesmen_medis_igd.gcs,asesmen_medis_igd.kesadaran,asesmen_medis_igd.td,asesmen_medis_igd.nadi,asesmen_medis_igd.rr,asesmen_medis_igd.suhu,asesmen_medis_igd.spo,"+
                        ...
                        "from reg_periksa inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                        "inner join asesmen_medis_igd on reg_periksa.no_rawat=asesmen_medis_igd.no_rawat "+
                        "inner join dokter on asesmen_medis_igd.kd_dokter=dokter.kd_dokter "+
                        "inner join bahasa_pasien on bahasa_pasien.id=pasien.bahasa_pasien "+
                        "inner join cacat_fisik on cacat_fisik.id=pasien.cacat_fisik where "+
                        "asesmen_medis_igd.tanggal between ? and ? order by asesmen_medis_igd.tanggal");
            }else{
                ps=koneksi.prepareStatement(
                        "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,if(pasien.jk='L','Laki-Laki','Perempuan') as jk,pasien.tgl_lahir,asesmen_medis_igd.tanggal,"+
                        "asesmen_medis_igd.jam,asesmen_medis_igd.kd_dokter,asesmen_medis_igd.anamnesis,asesmen_medis_igd.hubungan,asesmen_medis_igd.keluhan_utama,asesmen_medis_igd.rps,asesmen_medis_igd.rpk,asesmen_medis_igd.rpd,asesmen_medis_igd.rpo,asesmen_medis_igd.alergi,"+
                        ...
                        "inner join bahasa_pasien on bahasa_pasien.id=pasien.bahasa_pasien "+
                        "inner join cacat_fisik on cacat_fisik.id=pasien.cacat_fisik where "+
                        "asesmen_medis_igd.tanggal between ? and ? and reg_periksa.no_rawat like ? or "+
                        "asesmen_medis_igd.tanggal between ? and ? and pasien.no_rkm_medis like ? or "+
                        "asesmen_medis_igd.tanggal between ? and ? and pasien.nm_pasien like ? or "+
                        "asesmen_medis_igd.tanggal between ? and ? and asesmen_medis_igd.kd_dokter like ? or "+
                        "asesmen_medis_igd.tanggal between ? and ? and dokter.nm_dokter like ? order by asesmen_medis_igd.tanggal");
            }
    ...
    public void emptTeks() {
        ...
        try {
            ...
            "select * from penilaian_awal_keperawatan_igd where no_rawat=?" );
        ...
        try {
            ...
            "select * from data_triase_igd where no_rawat=?" );
        ...
        try {
            ...
            "SELECT * FROM periksa_lab,jns_perawatan_lab WHERE periksa_lab.kd_jenis_prw = jns_perawatan_lab.kd_jenis_prw AND periksa_lab.no_rawat=?" );
            
            try {
                ps.setString(1,TNoRw.getText());
                rs=ps.executeQuery();
                while(rs.next()){
                Laboratorium.setText("- "+rs.getString("nm_perawatan"));
        }
            } catch (Exception e) {
                System.out.println("Notif : "+e);
            }
        ...
        try {
            ...
            "select * from hasil_radiologi where no_rawat=?" );
            
            try {
                ps.setString(1,TNoRw.getText());
                rs=ps.executeQuery();
                while(rs.next()){
                Radiologi.setText("- "+rs.getString("hasil"));
        }
            } catch (Exception e) {
                System.out.println("Notif : "+e);
            }
        ...
        try {
            ...
            "SELECT GROUP_CONCAT(diagnosa_pasien.kd_penyakit,\" - \",nm_penyakit,\"\r\n\") as diagnosa FROM diagnosa_pasien,penyakit WHERE diagnosa_pasien.kd_penyakit = penyakit.kd_penyakit AND diagnosa_pasien.no_rawat=?" );
            
            try {
                ps.setString(1,TNoRw.getText());
                rs=ps.executeQuery();
                while(rs.next()){
                Diagnosis.setText(rs.getString("diagnosa"));
        }
            } catch (Exception e) {
                System.out.println("Notif : "+e);
            }
    ...
    private void getData() {
        if(tbObat.getSelectedRow()!= -1){
            ...
            urlImage=Sequel.cariIsi("select url_image from asesmen_medis_igd_image_marking where no_rawat='"+tbObat.getValueAt(tbObat.getSelectedRow(),0).toString()+"' ");
            imageAssesment("http://"+koneksiDB.HOSTHYBRIDWEB()+":"+koneksiDB.PORTWEB()+"/"+koneksiDB.HYBRIDWEB()+"/imagefreehand/"+urlImage+"");
...
//                ps=koneksi.prepareStatement(
//                        "select asesmen_medis_igd.tata from asesmen_medis_igd "+
//                        "where asesmen_medis_igd.no_rawat=? ");
//                try {
//                    ps.setString(1,tbObat.getValueAt(tbObat.getSelectedRow(),0).toString());
//                    rs=ps.executeQuery();
//                    while(rs.next()){
