 ...
 * @author salimmulyana
 */
public final class SuratGCRanap extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public SuratGCRanap(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        ...
        tabMode=new DefaultTableModel(null,new Object[]{
            "No.Surat","No.Rawat","No.R.M.","Nama Pasien","NIP","Petugas","Tanggal Surat","Jam","Hubungan Dengan Pasien","Pembayaran Obat","Kepada","Akses","Izin","Acc GC","Acc HK","Acc TT"
        }){
...
//                        KdPeg.requestFocus();  
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Persetujuan Umum/General Consent Rawat Inap ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 0, 11), new java.awt.Color(50, 50, 50))); // NOI18N
        ...
        TanggalSurat.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "12-01-2024" }));
        ...
        jLabel18.setText("Saya memberi kuasa kepada RSUD Matraman untuk memberikan informasi tentang diagnosis, hasil pelayanan dan pengobatan bila diperlukan untuk");
        ...
        Izin1.setEditable(false);
        Izin1.setHighlighter(null);
        ...
        FormInput.add(Izin1);
        Izin1.setBounds(430, 260, 250, 23);
        ...
        Hubungan.setBounds(430, 70, 300, 23);

        Obat.setEditable(false);
        ...
        FormInput.add(Obat);
        Obat.setBounds(480, 120, 250, 23);

        Hubungan3.setEditable(false);
        Hubungan3.setHighlighter(null);
        ...
        Hubungan3.setBounds(480, 120, 250, 23);

        jLabel22.setText("Saya Sudah Membaca, Mengerti dan Menyetujui “Hak dan Kewajiban Pasien”  ");
        jLabel22.setName("jLabel22"); // NOI18N
        FormInput.add(jLabel22);
        jLabel22.setBounds(6, 370, 390, 20);

        jLabel23.setText("Saya Sudah Membaca, Mengerti dan Menyetujui “General Consent”  ");
        jLabel23.setName("jLabel23"); // NOI18N
        FormInput.add(jLabel23);
        jLabel23.setBounds(10, 340, 340, 20);
        ...
        FormInput.add(acchk);
        acchk.setBounds(410, 370, 140, 24);

        accgc.setEditable(false);
        accgc.setHighlighter(null);
        accgc.setName("accgc"); // NOI18N
        accgc.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                accgcKeyPressed(evt);
            }
        });
        FormInput.add(accgc);
        accgc.setBounds(410, 340, 140, 24);
        ...
        jLabel24.setText("Saya Sudah Membaca, Mengerti dan Menyetujui “Tata Tertib”  ");
        jLabel24.setName("jLabel24"); // NOI18N
        FormInput.add(jLabel24);
        jLabel24.setBounds(15, 400, 310, 20);

        acctt.setEditable(false);
        acctt.setHighlighter(null);
        acctt.setName("acctt"); // NOI18N
        acctt.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                accttKeyPressed(evt);
            }
        });
        FormInput.add(acctt);
        acctt.setBounds(410, 400, 140, 24);
...
//        (Sequel.cariIsi("select no_rkm_medis from reg_periksa where no_rawat='"+TNoRw.getText()+"' ",TNoRM));
            ...
            if(Sequel.menyimpantf("surat_gc_ranap","?,?,?,?,?,?,?,?,?,?,?,?,?,?","No.Surat",14,new String[]{
                    NoSurat.getText(),TNoRw.getText(),KodeDokter.getText(),Valid.SetTgl(TanggalSurat.getSelectedItem()+""),jamNow.format(new Date()),"-","-","","-","","-","-","-",""
                })==true){
                tampil();
                emptTeks();
            }
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        Valid.hapusTable(tabMode,NoSurat,"surat_gc_ranap","no_surat");
        tampil();
        emptTeks();
...
//        }else if(TNoRw.getText().trim().equals("")||TPasien.getText().trim().equals("")){
            ...
            if(tbObat.getSelectedRow()!= -1){
                if(Sequel.mengedittf("surat_gc_ranap","no_surat=?","no_surat=?,no_rawat=?,nik=?,tanggal_surat=?,jam=?,hubungan=?,obat=?,kepada=?,izin1=?,izin=?,acc_gc=?,acc_hk=?,acc_tt=?,tte=?",15,new String[]{
                    NoSurat.getText(),TNoRw.getText(),KodeDokter.getText(),Valid.SetTgl(TanggalSurat.getSelectedItem()+""),jamNow.format(new Date()),Hubungan.getText(),Obat.getText(),Kepada.getText(),Izin1.getText(),Izin.getText(),accgc.getText(),acchk.getText(),acctt.getText(),tbObat.getValueAt(tbObat.getSelectedRow(),0).toString()
                })==true){
                    tampil();
                    emptTeks();
                }
        ...
        if(tabMode.getRowCount()==0){
            ...
            if(TCari.getText().trim().equals("")){
                Valid.MyReportqry("rptDataSuratSakit.jasper","report","::[ Data Surat Sakit Pasien ]::",
                     "select surat_gc_ranap.no_surat,surat_gc_ranap.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,"+
                     "surat_gc_ranap.tanggalawal,surat_gc_ranap.tanggalakhir,surat_gc_ranap.lamasakit "+                  
                     "from surat_gc_ranap inner join reg_periksa on surat_gc_ranap.no_rawat=reg_periksa.no_rawat "+
                     "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                     "where "+tgl+"order by surat_gc_ranap.no_surat",param);
            }else{
                Valid.MyReportqry("rptDataSuratSakit.jasper","report","::[ Data Surat Sakit Pasien ]::",
                     "select surat_gc_ranap.no_surat,surat_gc_ranap.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,"+
                     "surat_gc_ranap.tanggalawal,surat_gc_ranap.tanggalakhir,surat_gc_ranap.lamasakit "+                  
                     ...
                     tgl+"and surat_gc_ranap.no_rawat like '%"+TCari.getText().trim()+"%' or "+
                     tgl+"and reg_periksa.no_rkm_medis like '%"+TCari.getText().trim()+"%' or "+
                     tgl+"and pasien.nm_pasien like '%"+TCari.getText().trim()+"%' or "+
                     tgl+"and surat_gc_ranap.tanggalawal like '%"+TCari.getText().trim()+"%' or "+
                     tgl+"and surat_gc_ranap.tanggalakhir like '%"+TCari.getText().trim()+"%' "+
                     "order by surat_gc_ranap.no_surat",param);
            }
    ...
    private void MnCetakSuratSakitActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnCetakSuratSakitActionPerformed
       if(TPasien.getText().trim().equals("")){
            ...
            this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                ...
                param.put("hubungan",Sequel.cariIsi("select hubungan from surat_gc_ranap where no_rawat=?",TNoRw.getText()));
                param.put("obat",Sequel.cariIsi("select obat from surat_gc_ranap where no_rawat=?",TNoRw.getText()));
                param.put("kepada",Sequel.cariIsi("select kepada from surat_gc_ranap where no_rawat=?",TNoRw.getText()));
                param.put("izin1",Sequel.cariIsi("select izin1 from surat_gc_ranap where no_rawat=?",TNoRw.getText()));
                param.put("izin",Sequel.cariIsi("select izin from surat_gc_ranap where no_rawat=?",TNoRw.getText()));
                param.put("nik",Sequel.cariIsi("select pegawai.nama from surat_gc_ranap inner join pegawai on pegawai.nik=surat_gc_ranap.nik where no_rawat=?",TNoRw.getText()));
//                param.put("pandang",Pandang.getText());
//                param.put("kanan",Kanan.getText());
//                param.put("kiri",Kiri.getText());
//                param.put("rekomendasi",Rekomendasi.getText());
                param.put("namars",akses.getnamars());
                param.put("alamatrs",akses.getalamatrs());
                param.put("kotars",akses.getkabupatenrs());
                param.put("propinsirs",akses.getpropinsirs());
...
//        }
    ...
    private void accgcKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_accgcKeyPressed
        // TODO add your handling code here:
    }//GEN-LAST:event_accgcKeyPressed
    ...
    private void accttKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_accttKeyPressed
        // TODO add your handling code here:
    }//GEN-LAST:event_accttKeyPressed
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            SuratGCRanap dialog = new SuratGCRanap(new javax.swing.JFrame(), true);
    ...
    private widget.TextBox accgc;
    private widget.TextBox acchk;
    private widget.TextBox acctt;
    ...
    public void tampil() {
        ...
        try{
            tgl=" surat_gc_ranap.tanggal_surat between '"+Valid.SetTgl(DTPCari1.getSelectedItem()+"")+"' and '"+Valid.SetTgl(DTPCari2.getSelectedItem()+"")+"' ";
            if(TCari.getText().trim().equals("")){
                ps=koneksi.prepareStatement(
                     "select surat_gc_ranap.no_surat,surat_gc_ranap.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien, "+
                     "surat_gc_ranap.nik,pegawai.nama,surat_gc_ranap.tanggal_surat,surat_gc_ranap.jam,surat_gc_ranap.hubungan,surat_gc_ranap.obat,surat_gc_ranap.kepada, "+                  
                     "surat_gc_ranap.izin1,surat_gc_ranap.izin,surat_gc_ranap.acc_gc,surat_gc_ranap.acc_hk,surat_gc_ranap.acc_tt from surat_gc_ranap inner join reg_periksa on surat_gc_ranap.no_rawat=reg_periksa.no_rawat "+
                     "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                     "inner join pegawai on surat_gc_ranap.nik=pegawai.nik "+
                     "where "+tgl+"order by surat_gc_ranap.no_surat");
            }else{
                ps=koneksi.prepareStatement(
                     "select surat_gc_ranap.no_surat,surat_gc_ranap.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien, "+
                     "surat_gc_ranap.nik,pegawai.nama,surat_gc_ranap.tanggal_surat,surat_gc_ranap.jam,surat_gc_ranap.hubungan,surat_gc_ranap.obat,surat_gc_ranap.kepada, "+                  
                     ...
                     tgl+"and reg_periksa.no_rkm_medis like '%"+TCari.getText().trim()+"%' or "+
                     tgl+"and pasien.nm_pasien like '%"+TCari.getText().trim()+"%' or "+
                     tgl+"and surat_gc_ranap.hubungan like '%"+TCari.getText().trim()+"%' or "+
                     tgl+"and surat_gc_ranap.kepada like '%"+TCari.getText().trim()+"%' "+
                     "order by surat_gc_ranap.no_surat");
            }
    ...
    public void autoSurat(){
//    Valid.autoNomer3("select ifnull(MAX(CONVERT(RIGHT(surat_keterangan_rawat_inap.no_surat,3),signed)),0) from surat_keterangan_rawat_inap where surat_keterangan_rawat_inap.tanggalawal='"+Valid.SetTgl(TanggalSurat.getSelectedItem()+"")+"' ",
//                "GCRI"+TanggalSurat.getSelectedItem().toString().substring(0,2),3,NoSurat);
    Valid.autoNomer3("select ifnull(MAX(CONVERT(RIGHT(no_surat,4),signed)),0) from surat_gc_ranap where tanggal_surat='"+Valid.SetTgl(TanggalSurat.getSelectedItem()+"")+"' ","GCRI"+Valid.SetTgl(TanggalSurat.getSelectedItem()+"").replaceAll("-",""),5,NoSurat);
    }
...
//        KodeDokter.setText("");
//        Valid.autoNomer3("select ifnull(MAX(CONVERT(RIGHT(no_surat,3),signed)),0) from surat_gc_ranap where tanggal_surat='"+Valid.SetTgl(TanggalSurat.getSelectedItem()+"")+"' ",
//                "GC"+TanggalSurat.getSelectedItem().toString().substring(6,10)+TanggalSurat.getSelectedItem().toString().substring(3,5)+TanggalSurat.getSelectedItem().toString().substring(0,2),3,NoSurat); 
    ...
    private void getData() {
        if(tbObat.getSelectedRow()!= -1){
            ...
            Hubungan.setText(tbObat.getValueAt(tbObat.getSelectedRow(),8).toString());
            Obat.setText(tbObat.getValueAt(tbObat.getSelectedRow(),9).toString());
            Kepada.setText(tbObat.getValueAt(tbObat.getSelectedRow(),10).toString());
            Izin1.setText(tbObat.getValueAt(tbObat.getSelectedRow(),11).toString());
            Izin.setText(tbObat.getValueAt(tbObat.getSelectedRow(),12).toString());
            accgc.setText(tbObat.getValueAt(tbObat.getSelectedRow(),13).toString());
            acchk.setText(tbObat.getValueAt(tbObat.getSelectedRow(),14).toString());
            acctt.setText(tbObat.getValueAt(tbObat.getSelectedRow(),15).toString());
            panggilPhoto();
...
//        if(FormPhotoPass.isVisible()==true){
            try {
                ps=koneksi.prepareStatement("select surat_gc_ranap.tte from surat_gc_ranap where surat_gc_ranap.no_surat=?");
                try {
